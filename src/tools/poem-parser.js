/**
 * .poem source parser — the browser-safe core of the poem pipeline.
 *
 * `new PoemParser(text).parse()` turns .poem source text into a structured
 * poem-data object. The parse has no filesystem or Node-runtime coupling (fs
 * lives at the CLI boundary in poem-to-yaml.js), so this module is safe to
 * bundle for the browser — see src/browser/render.js. Keep it that way: do
 * NOT add fs/path/__dirname or any other Node-only dependency here.
 */
...
class PoemParser {
    ...
    while (true) {
      ...
      if (c === '"') {
        ...
        while (true) {
          ...
          if (dc === '$' && str[i + 1] === '{') {
            const expanded = expandVarAt(str, i, this.variables, this.usedBeforeDefined);
            if (expanded) {
              value += expanded.text;
  ...
  /**
   * Substitute author `${...}` variable references in `text`, against this
   * parser's own `this.variables`/`this.usedBeforeDefined` state. Delegates
   * to the pure scanner in poem-variables.js — see its docstring for the
   * substitution rules (nested/late binding, `:-default`, escaping, cycle
   * detection, the reserved `${!name}` form).
   *
   * @param {string} text
   * @returns {string}
   */
  substituteVariables(text) {
    return substituteVariablesPure(text, this.variables, this.usedBeforeDefined);
  }
